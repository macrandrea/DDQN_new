...
class Ambiente():
    ...
    def __init__(self, S0 = 10, mu = 0.0, kappa = 5, theta = 1, sigma = 0.01, lambd = 0.1, t0 = 0, t = 1, T = 3600, inv = 20): #T = 1.0, M = 7200, I = 1_000
...
class Agente():
    ...
    def __init__(self, inventario, numTrain):
        ...
        self.train = numTrain
        self.memory = ReplayMemory(5_000)   
        self.env = Ambiente()
    ...
    def action(self, state, min_p, max_p):
        ...
        elif np.random.uniform(0,1) <= self.epsilon and state[1] <= 4:

            coin = np.random.binomial(1,0.5)
            if coin == 0:

                action = (state[0] / (self.time_subdivisions + 1 - state[1]))
        ...
        #    action=np.random.binomial(state[0], 1 / (self.time_subdivisions - state[1]+1)) # B(n_t,p_s)
    ...
    def reward(self, inv, x, data):
        ...
        M = len(data)
        xs = x/M
        for i in range(1, M):#range(M*time+1,M*(time+1)):
            if i+1 < len(data):
    ...
    def train_1(self, transitions, data, p_min, p_max):
        ...
        for (s, next_s, rew, ac) in zip(state, next_state, reward, act):
            ...
            if next_s[1] == 4:
                ...
                best_future_action = self.q_action(next_s, p_min, p_max)
                target_value = rew + self.gamma * self.eval_Q(next_s, best_future_action, p_min, p_max, 'scalar', 'target')
                target.append(target_value)
    ...
    def test(self, inv, tempo, dati):
        ...
        return (state, x, next_state, reward)
    ...
    def PeL_QL(self, strategy, data):
        ...
        PeL = 0
        a = self.a_penalty
        M = len(data)#self.time_subdivisions
        #T_0 = self.starting_trading_time

        for i in range(self.time_subdivisions):

            x = self.lots_size * strategy[0][1]#strategy[i] * 
            xs = x / M

            for t in range(M):
                if t+1 < len(data):#if T_0 + i * M + t < len(data):
                    PeL += xs * data[t] - a * (xs ** 2) #T_0 + i * M +
    ...
    def PeL_TWAP(self, data):
        ...
        for i in range(self.time_subdivisions):
            ...
            for t in range(M):
                if t+1 < len(data):#T_0 + i * M + t < len(data):
                    PeL += xs * data[t] - a * (xs ** 2)#T_0 + i * M +
        return PeL
    ...
    def step(self, inv, tempo, data):
        ...
        #iter = 1
        self.timestep += 1
        state = [inv, tempo - 1, data[0]]
        p_min, p_max = data.min(), data.max()
        x = self.action(state, p_min, p_max)
        r = self.reward(inv, x, data)
        new_inv = inv - x
        next_state = [new_inv, tempo , data[-2]]
...
if __name__ == '__main__': 
    ...
    def doTrain(age, numIt = 200):
        ...
        data = Ambiente().abm(numIt=numIt) #------> questa la faccio da fuori?
        ...
        return (doAve(act_hist), doAve(loss_hist), doAve(rew_hist))
    ...
    def doTest(age, numIt = 100):
        ...
        for j in tqdm(range(numIt)):
            ...
            for i in tqdm(range(slices)):
                ...
                (current_state, x, next_state, reward) = age.test(inv, tempo, dati)
                
                inv = next_state[0]
                re.append(reward)
                act.append(x)
                selling_strategy.append([current_state, x, next_state, reward])
    ...
    def run():
        ...
        a, b, c = doTrain(age, numIt)
        pel, azioni, azioni_med, ricompensa = doTest(age, 1_000)

        return a, b, c, pel, azioni, azioni_med, ricompensa

    a, b, c, pel, azioni, azioni_med, ricompensa = run()

    print('average action chosen from train =', a, '\n' ,
    ', reward train =' , c,'\n',
     ',average loss from NN =', b, ',PeL performance=', pel,'\n' ,
     ',average actions test=', azioni_med, ',ricompensa = ', ricompensa)#, '\n', ',tutte le azioni di test=', azioni)
