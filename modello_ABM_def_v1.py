...
class DQN(nn.Module):
    '''
    fully connected feed-forward NN with 30 nodes each layer, for 6 layers
    '''
...
#
        ...
        loss = F.mse_loss(target.detach(), current_Q.reshape(-1,1))
...
if __name__ == '__main__': 
    ...
    def doTrain(age, numIt = 200):
        ...
        return (doAve(act_hist), act_hist, doAve(loss_hist), doAve(rew_hist))
    ...
    def run():
        ...
        a, a_t, b, c = doTrain(age, numIt)
        pel, azioni, azioni_med, ricompensa = doTest(age, 1_000)

        return a, a_t,  b, c, pel, azioni, azioni_med, ricompensa

    a, a_t,  b, c, pel, azioni, azioni_med, ricompensa = run()

    print('average action chosen from train =', a, ', reward train =' , c, ',average loss from NN =', b, 
